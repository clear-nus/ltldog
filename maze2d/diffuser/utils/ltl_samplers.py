...
def sample(path):
    ...
    ltl_sampler = 'Until_1_2_1_1'

    sampler = getLTLSampler(ltl_sampler, props)
...
def ltl_tuple2str(formula):
    operator_map = {'until': 'U',
                    'and': '&'}

    if isinstance(formula, tuple):
        operator = formula[0]
        if operator == 'not':
            # return '(~' + ltl_tuple2str(formula[1]) + ')'
            return '~' + ltl_tuple2str(formula[1])
        elif operator in ['and', 'or', 'implies', 'iff', 'until', 'release']:
            left = ltl_tuple2str(formula[1])
            right = ltl_tuple2str(formula[2])
...
if __name__ == '__main__':
    # sample("datasets/ltls_until.txt")
    read("datasets/ltls_until.txt")
