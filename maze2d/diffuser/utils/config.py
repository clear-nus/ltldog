...
def import_class(_class, output=print):
    ...
    output(f'[ utils/config ] Imported {repo_name}.{module_name}:{class_name}')
    return _class

class Config(collections.Mapping):

    def __init__(self, _class, verbose=True, savepath=None, output=print, device=None, **kwargs):
        self._class = import_class(_class, output)
        self._device = device
        self._dict = {}
        ...
        if savepath is not None:
            ...
            output(f'[ utils/config ] Saved config to: {savepath}\n')
